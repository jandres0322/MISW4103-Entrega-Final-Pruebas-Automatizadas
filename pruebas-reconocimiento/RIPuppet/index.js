...
async function recursiveExploration(page, link, depth, parentState){
  try {
    ...
    console.log('depthLevels: ', depthLevels, depth);
    ...
    if (page.url().includes('signin')) {
      ...
      await validateLogin(page, page.url());
      await delay(5000)
    ...
    if(!!body) {
      ...
      if(link.includes(baseUrl)){ //Only explore pages of the specified domain
        console.log("entrando a interactuar");
        let elementList = []; 
        ...
        //Taking screenshot
        console.log("External page: ", link);
        let imagePath = screenshots_directory + '/' + currentState + '.png';
        await page.screenshot({path: imagePath, fullPage: true});
      }
    }
...
async function validateLogin(page,link){
  await delay(3000);
  console.log("validateLogin link", link);
  if(link.includes('signin')){
    await page.goto(link);
    await page.waitForSelector('#identification'); 
    await page.type('#identification', "jhonandres0322@gmail.com"); 
    await page.waitForSelector('#password'); 
    await page.type('#password', "parceros123456"); 
